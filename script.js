...
class CommentsSystem {
  constructor() {
    this.elements = {
      ...
      visitorCounter: document.getElementById("visitor-count"),
    }
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 429) {
          const minutes = data.details.match(/\d+/)?.[0]
          if (minutes) this.updateButtonText(minutes)
  ...
  updateButtonText(minutes) {
    ...
    const text = minutes === "1" ? "> WAIT 1 MINUTE" : `> WAIT ${minutes} MINUTES`
    this.elements.submitButton.textContent = text
  }

  updateRateLimitState({ isLimited, remainingMinutes }) {
    if (this.state.rateLimitTimer) {
      clearInterval(this.state.rateLimitTimer)
      this.state.rateLimitTimer = null
    }
    ...
    this.setSubmitState(true)
    this.updateButtonText(remainingMinutes.toString())

    let minutes = remainingMinutes
    this.state.rateLimitTimer = setInterval(() => {
      minutes--
      if (minutes <= 0) {
        clearInterval(this.state.rateLimitTimer)
        this.state.rateLimitTimer = null
        ...
        this.updateButtonText(minutes.toString())
      }
    }, 60000)
  }

  updateVisitorCount(count) {
    if (this.elements.visitorCounter) {
      this.elements.visitorCounter.textContent = `Visitors: ${count || "..."}`
    }
  }
  ...
  renderComments(comments) {
    ...
    if (!comments.length) {
      this.elements.commentsList.innerHTML = '<div class="comment-item">No comments yet...</div>'
      return
    }
    ...
    ;[...comments].reverse().forEach((comment) => {
      ...
      `
      this.elements.commentsList.appendChild(div)
    })
...
class App {
  ...
  initializeComponents() {
    ...
    this.components.set("commentsSystem", new CommentsSystem())
  }
